...
export class EnhancedRiskMonitor {
  ...
  static assessRisk(
    ...
    // 1. 生成風險警報
    const activeAlerts = this.generateRiskAlerts(
      signals,
      indicators,
      scoringResult,
      marketCondition
  ...
  private static generateTechnicalAlerts(
    ...
    if (rsiIndicator && macdIndicator) {
      ...
      if (
        (rsiSignal === 'bullish' && macdSignal === 'bearish') ||
  ...
  private static generateMarketAlerts(
    marketCondition: string,
    scoringResult: DetailedScoringResult // eslint-disable-line @typescript-eslint/no-unused-vars
  ): RiskAlert[] {
    const alerts: RiskAlert[] = [];

    // 高波動市場警告
    if (
      marketCondition.includes('高波動') ||
      marketCondition.includes('劇烈')
    ) {
      alerts.push({
  ...
  private static generateOperationalAlerts(
    ...
    const typeDistribution = strategyTypes.reduce(
      (acc, type) => {
    ...
    const maxConcentration =
      Math.max(...Object.values(typeDistribution)) / signals.length;
  ...
  private static generateBehavioralAlerts(
    ...
    if (
      ...
      scoringResult.confidence === 'very_high'
    ) {
      alerts.push({
  ...
  private static identifyTechnicalRiskFactors(
    signals: StandardizedSignal[],
    indicators: IndicatorJudgment[] // eslint-disable-line @typescript-eslint/no-unused-vars
  ): RiskFactor[] {
    ...
    const hasConflict =
      signalDirections.includes('buy') && signalDirections.includes('sell');
  ...
  private static identifyMarketRiskFactors(
    marketCondition: string
  ): RiskFactor[] {
    const factors: RiskFactor[] = [];
  ...
  private static identifyOperationalRiskFactors(
    signals: StandardizedSignal[]
  ): RiskFactor[] {
    const factors: RiskFactor[] = [];
  ...
  private static identifyBehavioralRiskFactors(
    signals: StandardizedSignal[]
  ): RiskFactor[] {
    const factors: RiskFactor[] = [];
    ...
    const urgentBuySignals = buySignals.filter(
  ...
  private static calculateRiskMetrics(
    signals: StandardizedSignal[],
    scoringResult: DetailedScoringResult,
    portfolioContext?: any // eslint-disable-line @typescript-eslint/no-unused-vars
  ...
  private static estimateVolatility(signals: StandardizedSignal[]): number {
    ...
    const avgUrgency =
      signals.filter((s) => s.primary.urgency === 'immediate').length /
      signals.length;
  ...
  private static determineOverallRisk(
    alerts: RiskAlert[],
    riskFactors: any,
    riskMetrics: any // eslint-disable-line @typescript-eslint/no-unused-vars
