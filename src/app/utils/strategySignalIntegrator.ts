...
export class SignalConflictResolver {
  ...
  static resolveConflicts(signals: StandardizedSignal[]): {
    ...
    const conflictingPairs: ConflictPair[] = [];
    ...
    if (bullishSignals.length > 0 && bearishSignals.length > 0) {
      // 選擇強度最高的信號
      const strongestBullish = bullishSignals.sort(
        (a, b) => b.primary.strength - a.primary.strength
      )[0];
      const strongestBearish = bearishSignals.sort(
        (a, b) => b.primary.strength - a.primary.strength
      )[0];

      if (
        strongestBullish.primary.strength > strongestBearish.primary.strength
      ) {
        resolvedSignals = resolvedSignals.filter(
          (s) => s.primary.action !== 'sell'
        );
    ...
    // 2. 檢測時機衝突 (立即 vs 耐心等待)
    const immediateSignals = resolvedSignals.filter(
      (s) => s.primary.urgency === 'immediate'
    );
    const patientSignals = resolvedSignals.filter(
      (s) => s.primary.urgency === 'patient'
    );

    if (immediateSignals.length > 0 && patientSignals.length > 0) {
      // 根據信號強度決定時機
      const avgImmediateStrength =
        immediateSignals.reduce((sum, s) => sum + s.primary.strength, 0) /
        immediateSignals.length;
      const avgPatientStrength =
        patientSignals.reduce((sum, s) => sum + s.primary.strength, 0) /
        patientSignals.length;

      if (avgImmediateStrength > avgPatientStrength + 15) {
        // 立即行動信號更強
        resolvedSignals = resolvedSignals.map((s) =>
          s.primary.urgency === 'patient'
            ? { ...s, primary: { ...s.primary, urgency: 'normal' } }
            : s
        );
      } else {
        // 耐心等待更謹慎
        resolvedSignals = resolvedSignals.map((s) =>
          s.primary.urgency === 'immediate'
            ? { ...s, primary: { ...s.primary, urgency: 'normal' } }
            : s
        );
      }
    ...
    // 3. 檢測風險等級衝突
    const highRiskSignals = resolvedSignals.filter(
      (s) => s.strategy.riskLevel === 'high'
    );
    const lowRiskSignals = resolvedSignals.filter(
      (s) => s.strategy.riskLevel === 'low'
    );
...
export class AdvancedStrategyScorer {
  ...
  static calculateEnhancedScore(
    ...
    // 5. 市場適配度評分 (0-100)
    const marketFitScore = this.calculateMarketFitScore(
      signals,
      marketCondition
    );
  ...
  private static calculateSignalQuality(signals: StandardizedSignal[]): number {
    ...
    const strongSignalRatio =
      signals.filter((s) => s.primary.confidence === 'strong').length /
      signals.length;

    return Math.round(avgStrength * 0.7 + strongSignalRatio * 30);
  }

  private static calculateSignalConsistency(
    signals: StandardizedSignal[]
  ): number {
  ...
  private static calculateRiskAdjustedScore(
    signals: StandardizedSignal[]
  ): number {
    if (signals.length === 0) return 0;
  ...
  private static calculateTimingScore(
    ...
    if (macdIndicator) {
      if (
        macdIndicator.signal === 'bullish' ||
        macdIndicator.signal === 'bearish'
      ) {
  ...
  private static calculateMarketFitScore(
    signals: StandardizedSignal[],
    marketCondition: string
  ): number {
    if (signals.length === 0) return 0;
...
export class IntelligentAdviceGenerator {
  ...
  static generateAdvice(
    signals: StandardizedSignal[],
    scoring: OverallScoreResult,
    marketCondition: string
  ): EnhancedSignalAnalysis['actionRecommendation'] {
    const { resolvedSignals } =
      SignalConflictResolver.resolveConflicts(signals);

    // 分析主要動作
    const actionCounts = resolvedSignals.reduce(
      (acc, signal) => {
    ...
    const primaryAction = this.determinePrimaryAction(
      actionCounts,
  ...
  private static generateReasoning(
    ...
    if (scoring.overallScore >= 70) {
      ...
      reasoning.push(
        `綜合策略評分${scoring.overallScore}%，信號偏弱，需謹慎操作`
      );
    }
    ...
    // 3. 基於市場環境的推理
    reasoning.push(
      `當前市場狀況為${marketCondition}，${this.getMarketAdvice(marketCondition)}`
    );
  ...
  private static generateWarnings(
    ...
    // 2. 基於風險的警告
    const highRiskSignals = _signals.filter(
      (s: StandardizedSignal) => s.strategy.riskLevel === 'high'
    ).length;
...
export const performEnhancedStrategyAnalysis = (
  ...
  // 1. 解決信號衝突
  const { resolvedSignals, conflictReport } =
    SignalConflictResolver.resolveConflicts(signals);
  ...
  const signalStats = {
    ...
    bullishSignals: resolvedSignals.filter((s) => s.primary.action === 'buy')
      .length,
    bearishSignals: resolvedSignals.filter((s) => s.primary.action === 'sell')
      .length,
    neutralSignals: resolvedSignals.filter((s) => s.primary.action === 'hold')
      .length,
    conflictingSignals: conflictReport.hasConflicts,
    dominantDirection: actionRecommendation.primaryAction.includes('buy')
      ? ('bullish' as const)
      : actionRecommendation.primaryAction.includes('sell')
        ? ('bearish' as const)
        : ('neutral' as const),
  };
  ...
  const strengthAnalysis = {
    averageStrength:
      resolvedSignals.length > 0
        ? resolvedSignals.reduce((sum, s) => sum + s.primary.strength, 0) /
          resolvedSignals.length
        : 0,
    weightedStrength: scoring.subscores.riskAdjustedScore,
    strongSignalCount: resolvedSignals.filter(
      (s) => s.primary.confidence === 'strong'
    ).length,
    weakSignalCount: resolvedSignals.filter(
      (s) => s.primary.confidence === 'weak'
    ).length,
    strengthDistribution: resolvedSignals.reduce(
      (acc, s) => {
      ...
      resolvedSignals.length > 0
        ? Math.max(
            ...resolvedSignals.map(
              (s) =>
                STRATEGY_CONFIGS[
                  s.strategy.type as keyof typeof STRATEGY_CONFIGS
                ]?.maxDrawdown || 0.1
            )
          )
        : 0.1,
    positionSizeRecommendation: Math.min(
      0.5,
      Math.max(0.1, scoring.overallScore / 200)
    ),
  };
  ...
  const timingAnalysis = {
    optimalEntry:
      scoring.subscores.timingScore >= 75
        ? ('now' as const)
        : scoring.subscores.timingScore >= 60
          ? ('wait_confirmation' as const)
          : ('wait_dip' as const),
    timeHorizon: resolvedSignals.some((s) =>
      s.strategy.timeframe.includes('中期')
    )
