...
import {
  globalStrategySystem,
  type ComprehensiveAnalysisResult,
} from '@/utils/strategySystemController';
...
export const useStrategyEngine = (
  options: UseStrategyEngineOptions
  ...
  const { data: comprehensiveAnalysis, isLoading: isComprehensiveLoading } =
  ...
  const primaryRecommendation = useMemo(() => {
    ...
    const bestStrategy = strategies.reduce(
      (best, current) => (current.strength > best.strength ? current : best),
  ...
  const riskLevel = useMemo(() => {
    ...
    const highRiskStrategies = strategies.filter(
      (s) => s.riskLevel === 'high' && s.strength > 60
    );
    const avgRisk =
  ...
  const actionAdvice = useMemo(() => {
    ...
    return {
      ...
      warnings: [...template.warnings, ...finalRecommendation.riskWarnings],
  ...
  const getIndicatorStatus = useCallback(
    (indicator: string): IndicatorJudgment | null => {
  ...
  const getStrategyByType = useCallback(
    (type: string): StrategySignal | null => {
 ...
 * 獲取特定指標的歷史趨勢
 */
export const useIndicatorTrend = (
  symbol: string,
  indicator: string,
  timeRange: string
) => {
