...
import type { StrategyAnalysis } from './strategyEngine';
...
import { RuleEngine } from './strategyRuleEngine';
...
export class StrategySystemController {
  ...
  async performComprehensiveAnalysis(
    ...
    try {
      ...
      const standardizedSignals = SignalFormatter.formatMultipleSignals(
        ruleResults,
        symbol,
        currentPrice
      );
      ...
      const riskAssessment = this.config.analysis.enableRiskMonitoring
        ? EnhancedRiskMonitor.assessRisk(
            ...
            portfolioContext
              ? {
                  ...portfolioContext,
                  stopLossLevels: portfolioContext.stopLossLevels || {},
                }
              : undefined
          )
      ...
      const intelligentAdvice = this.config.analysis.enableIntelligentAdvice
        ? IntelligentAdviceEngine.generateAdvice(
            ...
            portfolioContext
              ? {
                  ...portfolioContext,
                  timeHorizon: portfolioContext.timeHorizon || 'medium',
                }
              : undefined
          )
  ...
  performQuickAnalysis(
    ...
    // 風險等級評估
    const riskLevel =
      overallStrength >= 70 ? 'low' : overallStrength >= 50 ? 'medium' : 'high';
  ...
  private filterSignals(signals: StandardizedSignal[]): StandardizedSignal[] {
    ...
    filtered = filtered.filter(
      (s) => s.primary.strength >= this.config.strategies.minimumSignalStrength
    );

    // 按啟用的策略類型過濾
    filtered = filtered.filter((s) => {
      // 映射策略類型
      let strategyType = s.strategy.type;
      if (strategyType === 'reversal') strategyType = 'mean_reversion';
      if (strategyType === 'trend') strategyType = 'momentum';

      return this.config.strategies.enabledTypes.includes(
        strategyType as 'momentum' | 'mean_reversion' | 'breakout'
      );
    });
  ...
  private createBasicScoring(
    signals: StandardizedSignal[]
  ): DetailedScoringResult {
    const avgStrength =
      signals.length > 0
        ? signals.reduce((sum, s) => sum + s.primary.strength, 0) /
          signals.length
        : 0;
    ...
    return {
      ...
      confidence:
        avgStrength >= 70 ? 'high' : avgStrength >= 50 ? 'medium' : 'low',
  ...
  private createBasicRiskAssessment(): RiskAssessmentResult {
    return {
      ...
      riskBudget: {
        ...
        available: 0.1,
        utilization: 33,
        limits: { single_position: 0.05, sector: 0.1, strategy: 0.08 },
      },
    };
  }
  ...
  private generateOverallAssessment(
    ...
    return {
      ...
      confidenceScore: Math.round(
        (scoring.overallScore + (100 - risk.riskScore)) / 2
      ),
  ...
  private generateExecutionGuidance(
    ...
    const priorityActions = [
      {
        action:
          advice.primaryAdvice.action === 'hold'
            ? '保持當前倉位'
            : advice.primaryAdvice.action.includes('buy')
              ? '考慮建倉'
              : '考慮減倉',
        timeline:
          advice.primaryAdvice.urgency === 'immediate' ? '立即' : '1-3個交易日',
        importance:
          assessment.recommendationLevel === 'strong_recommend'
            ? ('critical' as const)
            : ('medium' as const),
  ...
  private generateMonitoringPlan(
    ...
    return {
      dailyChecks: ['檢查價格變動', '監控關鍵技術位', '觀察成交量變化'],
      weeklyReviews: ['檢視策略表現', '評估風險指標', '調整倉位配置'],
      monthlyAssessments: ['全面策略回顧', '風險管理檢討', '投資組合優化'],
  ...
  private generateQuickReasoning(analysis: StrategyAnalysis): string {
    const { overallSignal, overallStrength } = analysis;
    const strongIndicators = analysis.indicatorJudgments.filter(
      (i) => i.confidence === 'strong'
    ).length;

    return (
      `基於技術分析，當前信號${overallSignal === 'bullish' ? '偏向看漲' : overallSignal === 'bearish' ? '偏向看跌' : '中性'}，` +
...
export const analyzeStock = async (
  ...
  return globalStrategySystem.performComprehensiveAnalysis(
    data,
    symbol,
    currentPrice
  );
 ...
 * 便捷的快速分析方法
 */
export const quickAnalyzeStock = (data: StockAnalysisDTO, symbol: string) => {
  return globalStrategySystem.performQuickAnalysis(data, symbol);
};
