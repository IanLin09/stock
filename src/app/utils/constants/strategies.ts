...
import type { StrategyType, RiskLevel, ConfidenceLevel } from '../strategyEngine';
...
export const SIGNAL_STRENGTH = {
  ...
  risk_levels: {
    low: { label: '低風險', color: '#10b981', description: '相對安全，適合保守投資者' },
    medium: { label: '中風險', color: '#f59e0b', description: '風險適中，需要適當管理' },
    high: { label: '高風險', color: '#ef4444', description: '高風險高收益，需要嚴格止損' }
  }
} as const;
...
export const getSignalColor = (signal: string) => {
  return SIGNAL_STRENGTH.signal_types[signal as keyof typeof SIGNAL_STRENGTH.signal_types]?.color || '#6b7280';
};
