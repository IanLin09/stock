...
export class EnhancedScoringEngine {
  ...
  static performDetailedScoring(
    ...
    const componentScores = {
      ...
      indicatorAlignment: this.calculateIndicatorAlignmentScore(
        signals,
        indicators
      ),
      riskAdjustment: this.calculateRiskAdjustmentScore(signals),
      timingOptimality: this.calculateTimingOptimalityScore(
        signals,
        indicators
      ),
      marketConditionFit: this.calculateMarketFitScore(
        signals,
        marketCondition
      ),
      volumeConfirmation: this.calculateVolumeConfirmationScore(
        signals,
        historicalData
      ),
      historicalAccuracy: this.calculateHistoricalAccuracyScore(
        signals,
        historicalData
      ),
    };
    ...
    // 3. 確定信心等級和評級
    const confidence = this.determineConfidenceLevel(
      overallScore,
      componentScores
    );
   ...
   * 計算信號強度評分
   */
  private static calculateSignalStrengthScore(
    signals: StandardizedSignal[]
  ): number {
    ...
    return Math.min(
      100,
      Math.round(avgStrength * 0.7 + strongSignalRatio * 20 + signalCountBonus)
    );
  }
  ...
  private static calculateIndicatorAlignmentScore(
    ...
    indicators.forEach((indicator) => {
      ...
      // 計算該指標與策略的一致性
      const consistency = this.calculateIndicatorSignalConsistency(
        indicator,
        signals
      );
   ...
   * 計算風險調整評分
   */
  private static calculateRiskAdjustmentScore(
    signals: StandardizedSignal[]
  ): number {
    if (signals.length === 0) return 0;

    // 計算風險分布
    const riskCounts = signals.reduce(
      (acc, s) => {
  ...
  private static calculateTimingOptimalityScore(
    ...
    if (rsiIndicator) {
      if (rsiIndicator.signal === 'extreme') {
        ...
        rsiIndicator.signal === 'bullish' ||
        rsiIndicator.signal === 'bearish'
      ) {
        timingScore += 15;
      }
    }
    ...
    if (
      macdIndicator &&
      (macdIndicator.signal === 'bullish' || macdIndicator.signal === 'bearish')
    ) {
      timingScore += 15;
    }
  ...
  private static calculateMarketFitScore(
    ...
    // 趨勢市場偏好動量策略
    if (
      marketCondition.includes('趨勢') ||
      marketCondition.includes('上漲') ||
      marketCondition.includes('下跌')
    ) {
    ...
    if (marketCondition.includes('突破')) {
      const breakoutSignals = signals.filter((s) =>
        s.strategy.name.includes('突破')
      ).length;
  ...
  private static getScoreDescription(
    category: string,
    ...
    return (
      descriptions[category as keyof typeof descriptions]?.[type] || '評分異常'
    );
  }
  ...
  private static calculateRiskMetrics(
    ...
    const winRate = Math.min(
      0.85,
      Math.max(0.45, (overallScore / 100) * 0.7 + 0.15)
    );
  ...
  private static generateActionGuidance(
    ...
    // Map action to recommended action
    ...
    if (topAction === 'buy' || topAction === 'sell' || topAction === 'hold') {
