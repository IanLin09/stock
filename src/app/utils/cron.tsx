  ...
  try {
    ...
    const responses = await Promise.all([
        fetch(`${process.env.NEXT_PUBLIC_API}/indicators/latest`, {
            method: 'POST',headers: { Authorization: `Bearer ${process.env.NEXT_PUBLIC_AWSTOKEN}` },body: JSON.stringify({ symbol:"QQQ" }),
         }),
        fetch(`${process.env.NEXT_PUBLIC_API}/daily/create`, {
            method: 'POST',headers: { Authorization: `Bearer ${process.env.NEXT_PUBLIC_AWSTOKEN}` },
        }),
        ...symbols.map((symbol) =>
